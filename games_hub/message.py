...
# error message
CONFIG_NOT_EXIST_ERROR_MSG = f"Cannot read {CONFIG_PATH}!"
TELEGRAM_REQUIRE_TOKEN_ERROR_MSG = (
    f"Cannot get token of telegram from {CONFIG_PATH}!"
)
TELEGRAM_REQUIRE_CHAT_ID_ERROR_MSG = (
    f"Cannot get chat_id of telegram from {CONFIG_PATH}!"
)
ASF_REQUIRE_IPC_ERROR_MSG = f"Cannot get ipc of asf from {CONFIG_PATH}!"
AT_LEAST_ENABLE_ONE_FUNCTION_ERROR_MSG = "Both telegram and asf are not enabled!"
CONFIG_FILE_NEEDS_TO_BE_UPDATED = (
    f"{CONFIG_PATH}'s format has been changed, please change it accordingly!"
)
